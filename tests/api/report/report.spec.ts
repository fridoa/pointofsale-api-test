...
test.describe.serial('Report Flow', () => {
  ...
  test('RPT-01: Get Sales Summary (Admin)', { tag: ['@smoke', '@regression'] }, async ({ request }) => {
    ...
    await test.step('3. Verify calculations', async () => {
      ...
      const expectedMargin = (expectedProfit / expectedRevenue) * 100; // 40

      // We use toBeGreaterThanOrEqual because there might be other transactions from other tests running globally 
      // if not fully cleaned up, but at minimum it should have our transaction's value.
      // Wait, global setup cleans all transactions! So it should EXACTLY be this, unless tests run in parallel.
      // Playwright is set to fullyParallel: false, workers: 1. So it should match exactly, 
      // but to be safe with precision issues we'll just check it includes our transaction value.
